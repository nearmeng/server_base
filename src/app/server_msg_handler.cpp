...
static Message* get_msg_inst(int32_t nMsgID)
{
	...
	MSGID_TO_STR pMsgFunc = CMGApp::instance().get_config().pMsgFunc;
	...
	LOG_PROCESS_ERROR(pMsgFunc);
	sMsgName = pMsgFunc(nMsgID, mdtCS);
...
BOOL recv_client_msg_proc(uint64_t qwConnID, const char* pBuff, int32_t nSize)
{
    ...
    {
        std::string sMsgStr;
        std::string sMsgName = pMsgFunc(CsHead.msgid(), mdtCS);
...
BOOL broadcast_to_client(int32_t nMsgID, const Message* pMsg)
{
    ...
    {
        std::string sMsgStr;
        std::string sMsgName = pMsgFunc(nMsgID, mdtSC);
    ...
    nRetCode = send_server_msg_by_service_type(g_BaseConfig.nConnServerType, conn_broadcast_msg, msg, nAllDataSize);
    LOG_PROCESS_ERROR(nRetCode);
