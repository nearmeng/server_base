...
BOOL CBTMgr::init(REG_BT_OWNER_FUNC pBtOwnerFunc, int32_t nInitBtCtxCount, BOOL bResume)
{
	int32_t nRetCode = 0;

	LOG_PROCESS_ERROR(pBtOwnerFunc);
	
	nRetCode = m_BtCtx.init(stdBtCtx, nInitBtCtxCount, bResume);
...
BOOL CBTMgr::register_owner_data(int32_t nOwnerType, const char* pcszName, GET_OWNER pGetOwnerFunc, GET_OWNER_VAR pGetOwnerVarFunc)
{
	...
	LOG_PROCESS_ERROR(nOwnerType > 0 && nOwnerType < MAX_OWNER_TYPE_COUNT);
