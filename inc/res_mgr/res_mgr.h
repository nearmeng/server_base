...
private:
    ...
    char                        m_szConfigName[CONFIG_NAME_LEN];
    char                        m_szObjName[OBJ_NAME_LEN];
...
{
    int32_t nRetCode = 0;
    char szFileName[RES_NAME_LEN];
    int32_t nResMode = CGlobalResMgr::instance().get_res_mode();
    
    switch (nResMode)
    {
    case rlmTab:
        snprintf(szFileName, RES_NAME_LEN, "%s/%s.txt", m_szPath, m_szConfigName);
        nRetCode = oResFile.init(szFileName, '\t');
        LOG_PROCESS_ERROR(nRetCode);
        break;
    case rlmCsv:
        snprintf(szFileName, RES_NAME_LEN, "%s/%s.csv", m_szPath, m_szConfigName);
        nRetCode = oResFile.init(szFileName, ',');
        LOG_PROCESS_ERROR(nRetCode);
        break;
    default:
...
BOOL CResMgr<T>::_read_line(CResFile& oResFile, int32_t nRowIndex, T* data)
{
    ...
    for (int32_t nColIndex = 0; nColIndex < oResFile.get_col_count(); nColIndex++)
    {
        ...
        char szArrayIndex[RES_NAME_LEN] = { 0 };
        char szArrayName[RES_NAME_LEN] = { 0 };
        ...
        nColNameLen = strnlen(pcszColName, RES_NAME_LEN);
        //array support
