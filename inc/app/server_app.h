...
typedef std::string(*MSGID_TO_STR)(int32_t nMsgID, MSG_DIRECTION_TYPE nType);
...
struct MG_CONFIG
{
	...
	std::vector<TOLUA_FUNC> vToluaFunc;
...
public:
	...
	inline MG_CONFIG& get_config(void);
...
private:
    ...
    USER_MSG_HANDLER m_UserMsgHandler[MAX_INTERACT_SERVER_COUNT];
...
inline MG_CONFIG& CMGApp::get_config(void) 
{
	return m_Config;
}
